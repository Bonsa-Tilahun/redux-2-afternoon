...
export function requestBudgetData() {
    const responsePromise = axios.get('/api/budget-data').then(res => res.data)
    return {
...
export function addPurchase(price, description, category) {
    const addPromise = axios.post('/api/budget-data/purchase', { description, price, category }).then(res => res.data)
    return {
        type: ADD_PURCHASE,
        payload: addPromise
    }
